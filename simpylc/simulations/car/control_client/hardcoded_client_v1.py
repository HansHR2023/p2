...
class HardcodedClient:
    def __init__ (self):
        ...
        self.model = torch.load('../../../../notebooks/data/sonar_model_weights.pth')
    ...
    def sonarSweep (self):
        ...
        self.steeringAngle = model(sensors)
        self.targetVelocity = pm.getTargetVelocity (self.steeringAngle)
    ...
    def predict(self):
        ...
        for sonarAngle in range (-self.halfApertureAngle, self.halfApertureAngle):
            sectorIndex = round (sonarAngle / self.sectorAngle)
            sample [sectorIndex] = min (sample [sectorIndex], self.sonarDistances [sonarAngle])
